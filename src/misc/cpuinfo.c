...
#elif MIL_ALTIVEC_BLITTERS && HAVE_SETJMP
#include <signal.h>
#include <setjmp.h>
#endif
...
#if MIL_ALTIVEC_BLITTERS && HAVE_SETJMP && !__MACOSX__
...
static Uint32 MIL_CPUFeatures = 0xFFFFFFFF;

static Uint32 MIL_GetCPUFeatures(void)
{
	if ( MIL_CPUFeatures == 0xFFFFFFFF ) {
		MIL_CPUFeatures = 0;
		if ( CPU_haveRDTSC() ) {
			MIL_CPUFeatures |= CPU_HAS_RDTSC;
		}
		if ( CPU_haveMMX() ) {
			MIL_CPUFeatures |= CPU_HAS_MMX;
		}
		if ( CPU_haveMMXExt() ) {
			MIL_CPUFeatures |= CPU_HAS_MMXEXT;
		}
		if ( CPU_have3DNow() ) {
			MIL_CPUFeatures |= CPU_HAS_3DNOW;
		}
		if ( CPU_have3DNowExt() ) {
			MIL_CPUFeatures |= CPU_HAS_3DNOWEXT;
		}
		if ( CPU_haveSSE() ) {
			MIL_CPUFeatures |= CPU_HAS_SSE;
		}
		if ( CPU_haveSSE2() ) {
			MIL_CPUFeatures |= CPU_HAS_SSE2;
		}
		if ( CPU_haveAltiVec() ) {
			MIL_CPUFeatures |= CPU_HAS_ALTIVEC;
		}
	}
