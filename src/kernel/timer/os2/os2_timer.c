...
DECLSPEC Uint32 MILCALL MIL_GetTicks(void)
{
        long long hires_now;
        ULONG ticks = ticks;
...
DECLSPEC void MILCALL MIL_Delay(Uint32 ms)
{
 ...
 /* tib->tib_ptib2->tib2_ulpri = priority; */ /* Get back... */
  ...
  if (e)
  {
    MIL_SetError("[MIL_Delay] : Had error in %s(), rc is 0x%x\n", e, badrc);
  }
}
...
static int MILCALL RunTimer(void *unused)
{
        DosSetPriority(PRTYS_THREAD, PRTYC_TIMECRITICAL, 0, 0);
        while ( timer_alive ) {
...
{
        MIL_SetError("Internal logic error: OS/2 uses threaded timer");
        return(-1);
}
