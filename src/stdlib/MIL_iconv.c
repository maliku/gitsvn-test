...
/* This file contains portable iconv functions for MIL */
...
size_t MIL_iconv(MIL_iconv_t cd,
                 ...
                 char **outbuf, size_t *outbytesleft)
{
	...
	if ( retCode == (size_t)-1 ) {
		switch(errno) {
		    case E2BIG:
			return MIL_ICONV_E2BIG;
		    case EILSEQ:
			return MIL_ICONV_EILSEQ;
		    case EINVAL:
			return MIL_ICONV_EINVAL;
		    default:
			return MIL_ICONV_ERROR;
		}
	}
	return retCode;
}
...
static const char *getlocale(char *buffer, size_t bufsize)
{
	...
	lang = MIL_getenv("LC_ALL");
	if ( !lang ) {
		lang = MIL_getenv("LC_CTYPE");
	}
	if ( !lang ) {
		lang = MIL_getenv("LC_MESSAGES");
	}
	if ( !lang ) {
		lang = MIL_getenv("LANG");
	}
	if ( !lang || !*lang || MIL_strcmp(lang, "C") == 0 ) {
		lang = "ASCII";
	}
	...
	MIL_strlcpy(buffer, lang, bufsize);
	ptr = MIL_strchr(buffer, '@');
	if (ptr != NULL) {
...
MIL_iconv_t MIL_iconv_open(const char *tocode, const char *fromcode)
{
	...
	for ( i = 0; i < MIL_arraysize(encodings); ++i ) {
		if ( MIL_strcasecmp(fromcode, encodings[i].name) == 0 ) {
			src_fmt = encodings[i].format;
			if ( dst_fmt != ENCODING_UNKNOWN ) {
				break;
			}
		}
		if ( MIL_strcasecmp(tocode, encodings[i].name) == 0 ) {
	...
	if ( src_fmt != ENCODING_UNKNOWN && dst_fmt != ENCODING_UNKNOWN ) {
		MIL_iconv_t cd = (MIL_iconv_t)MIL_malloc(sizeof(*cd));
		if ( cd ) {
			cd->src_fmt = src_fmt;
			cd->dst_fmt = dst_fmt;
			return cd;
...
int MIL_iconv_close(MIL_iconv_t cd)
{
	if ( cd && cd != (MIL_iconv_t)-1 ) {
		MIL_free(cd);
	}
	return 0;
}
...
char *MIL_iconv_string(const char *tocode, const char *fromcode, const char *inbuf, size_t inbytesleft)
{
	MIL_iconv_t cd;
	char *string;
	size_t stringsize;
	...
	cd = MIL_iconv_open(tocode, fromcode);
	if ( cd == (MIL_iconv_t)-1 ) {
	...
	MIL_memset(outbuf, 0, 4);

	while ( inbytesleft > 0 ) {
		retCode = MIL_iconv(cd, &inbuf, &inbytesleft, &outbuf, &outbytesleft);
		switch (retCode) {
		    case MIL_ICONV_E2BIG:
			{
				char *oldstring = string;
				stringsize *= 2;
				string = MIL_realloc(string, stringsize);
				if ( !string ) {
					MIL_iconv_close(cd);
					return NULL;
				}
			...
			break;
		    case MIL_ICONV_EILSEQ:
			/* Try skipping some input data - not perfect, but... */
			++inbuf;
			--inbytesleft;
			break;
		    case MIL_ICONV_EINVAL:
		    case MIL_ICONV_ERROR:
			/* We can't continue... */
			inbytesleft = 0;
			break;
		}
