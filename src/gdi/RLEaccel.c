...
do {							\
    if(bpp == 4) {					\
	MIL_memcpy4(to, from, (size_t)(len));		\
    } else {						\
	MIL_memcpy(to, from, (size_t)(len) * (bpp));	\
    }							\
} while(0)
...
#undef RLESKIP
	...
	alpha = (src->flags & MIL_SRCALPHA) == MIL_SRCALPHA
...
static int RLEAlphaSurface(Surface *surface)
{
    ...
    rlebuf = (Uint8 *)MIL_malloc(maxsize);
    if(!rlebuf) {
	MIL_OutOfMemory();
	return -1;
    }
...
#undef ADD_TRANSL_COUNTS
    ...
    if((surface->flags & MIL_PREALLOC) != MIL_PREALLOC
    ...
    {
	Uint8 *p = MIL_realloc(rlebuf, dst - rlebuf);
	if(!p)
	    p = rlebuf;
...
#define ADD_COUNTS(n, m)			\
	...
	for(y = 0; y < h; y++) {
		...
		len = MIN(run, maxn);
		ADD_COUNTS(skip, len);
		MIL_memcpy(dst, srcbuf + runstart * bpp, len * bpp);
		dst += len * bpp;
		run -= len;
		runstart += len;
		while(run) {
