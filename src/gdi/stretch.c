...
*/
     ...
     defined(__WATCOMC__) || \
     (defined(__GNUC__) && defined(__i386__))) && MIL_ASSEMBLY_ROUTINES
...
static int generate_rowbytes(int src_w, int dst_w, int bpp)
{
	...
	switch (bpp) {
		...
		break;
	    default:
		MIL_SetError("ASM stretch of %d bytes isn't supported\n", bpp);
		return(-1);
	}
#ifdef HAVE_MPROTECT
	/* Make the code writeable */
	if ( mprotect(copy_row, sizeof(copy_row), PROT_READ|PROT_WRITE) < 0 ) {
		MIL_SetError("Couldn't make copy buffer writeable");
		return(-1);
	}
#endif
	pos = 0x10000;
	...
	*eip++ = RETURN;
	...
	if ( eip > (copy_row+sizeof(copy_row)) ) {
		MIL_SetError("Copy buffer overflow");
		return(-1);
	}
#ifdef HAVE_MPROTECT
	/* Make the code executable but not writeable */
	if ( mprotect(copy_row, sizeof(copy_row), PROT_READ|PROT_EXEC) < 0 ) {
		MIL_SetError("Couldn't make copy buffer executable");
		return(-1);
	}
#endif
	last.status = 0;
                    ...
                    Surface *dst, MIL_Rect *dstrect)
{
	...
	if ( src->format->BitsPerPixel != dst->format->BitsPerPixel ) {
		MIL_SetError("Only works with same format surfaces");
		return(-1);
	}
	...
	if ( srcrect ) {
		if ( (srcrect->x < 0) || (srcrect->y < 0) ||
			...
			MIL_SetError("Invalid source blit rectangle");
			return(-1);
		}
	} else {
		full_src.x = 0;
	...
	if ( dstrect ) {
		if ( (dstrect->x < 0) || (dstrect->y < 0) ||
			...
			MIL_SetError("Invalid destination blit rectangle");
			return(-1);
		}
	} else {
		full_dst.x = 0;
