...
static void BlitNto1SurfaceAlpha(MIL_BlitInfo *info)
{
...
static void BlitNto1PixelAlpha(MIL_BlitInfo *info)
{
	int width = info->d_width;
	int height = info->d_height;
	Uint8 *src = info->s_pixels;
...
static void BlitNto1SurfaceAlphaKey(MIL_BlitInfo *info)
{
	int width = info->d_width;
	int height = info->d_height;
	Uint8 *src = info->s_pixels;
...
static void BlitRGBtoRGBSurfaceAlpha128MMX(MIL_BlitInfo *info)
{
	int width = info->d_width;
	int height = info->d_height;
...
static void BlitRGBtoRGBSurfaceAlphaMMX(MIL_BlitInfo *info)
{
	MIL_PixelFormat* df = info->dst;
...
static void BlitRGBtoRGBPixelAlphaMMX(MIL_BlitInfo *info)
{
	int width = info->d_width;
	int height = info->d_height;
	Uint32 *srcp = (Uint32 *)info->s_pixels;
...
static void Blit32to565PixelAlphaAltivec(MIL_BlitInfo *info)
{
    int height = info->d_height;
...
static void Blit32to32SurfaceAlphaKeyAltivec(MIL_BlitInfo *info)
{
    unsigned alpha = info->src->alpha;
    int height = info->d_height;
    Uint32 *srcp = (Uint32 *)info->s_pixels;
    ...
    unsigned dA = dstfmt->Amask ? MIL_ALPHA_OPAQUE : 0;
...
static void Blit32to32PixelAlphaAltivec(MIL_BlitInfo *info)
{
    int width = info->d_width;
    int height = info->d_height;
    Uint32 *srcp = (Uint32 *)info->s_pixels;
...
/* fast ARGB888->(A)RGB888 blending with pixel alpha */
static void BlitRGBtoRGBPixelAlphaAltivec(MIL_BlitInfo *info)
{
	int width = info->d_width;
	int height = info->d_height;
	Uint32 *srcp = (Uint32 *)info->s_pixels;
...
static void Blit32to32SurfaceAlphaAltivec(MIL_BlitInfo *info)
{
    /* XXX : 6 */
	unsigned alpha = info->src->alpha;
    int height = info->d_height;
...
static void BlitRGBtoRGBSurfaceAlphaAltivec(MIL_BlitInfo *info)
{
	unsigned alpha = info->src->alpha;
    int height = info->d_height;
    Uint32 *srcp = (Uint32 *)info->s_pixels;
...
#endif
#endif /* MIL_ALTIVEC_BLITTERS */

/* fast RGB888->(A)RGB888 blending with surface alpha=128 special case */
static void BlitRGBtoRGBSurfaceAlpha128(MIL_BlitInfo *info)
{
	int width = info->d_width;
	int height = info->d_height;
	Uint32 *srcp = (Uint32 *)info->s_pixels;
...
/* fast RGB888->(A)RGB888 blending with surface alpha */
static void BlitRGBtoRGBSurfaceAlpha(MIL_BlitInfo *info)
{
	unsigned alpha = info->src->alpha;
	if(alpha == 128) {
...
/* fast ARGB888->(A)RGB888 blending with pixel alpha */
static void BlitRGBtoRGBPixelAlpha(MIL_BlitInfo *info)
{
	int width = info->d_width;
	int height = info->d_height;
	Uint32 *srcp = (Uint32 *)info->s_pixels;
...
static void BlitRGBtoRGBPixelAlphaMMX3DNOW(MIL_BlitInfo *info)
{
	int width = info->d_width;
	int height = info->d_height;
	Uint32 *srcp = (Uint32 *)info->s_pixels;
...
static void Blit16to16SurfaceAlpha128(MIL_BlitInfo *info, Uint16 mask)
{
	int width = info->d_width;
	int height = info->d_height;
...
static void Blit565to565SurfaceAlphaMMX(MIL_BlitInfo *info)
{
...
static void Blit555to555SurfaceAlphaMMX(MIL_BlitInfo *info)
{
	unsigned alpha = info->src->alpha; /* downscale alpha to 5 bits */
	if(alpha == 128) {
...
/* fast RGB565->RGB565 blending with surface alpha */
static void Blit565to565SurfaceAlpha(MIL_BlitInfo *info)
{
	unsigned alpha = info->src->alpha;
	if(alpha == 128) {
		Blit16to16SurfaceAlpha128(info, 0xf7de);
...
/* fast RGB555->RGB555 blending with surface alpha */
static void Blit555to555SurfaceAlpha(MIL_BlitInfo *info)
{
	unsigned alpha = info->src->alpha; /* downscale alpha to 5 bits */
	if(alpha == 128) {
		Blit16to16SurfaceAlpha128(info, 0xfbde);
...
static void BlitARGBto565PixelAlpha(MIL_BlitInfo *info)
{
	int width = info->d_width;
	int height = info->d_height;
	Uint32 *srcp = (Uint32 *)info->s_pixels;
...
static void BlitARGBto555PixelAlpha(MIL_BlitInfo *info)
{
	int width = info->d_width;
	int height = info->d_height;
	Uint32 *srcp = (Uint32 *)info->s_pixels;
...
static void BlitNtoNSurfaceAlpha(MIL_BlitInfo *info)
{
	int width = info->d_width;
	int height = info->d_height;
	Uint8 *src = info->s_pixels;
...
static void BlitNtoNSurfaceAlphaKey(MIL_BlitInfo *info)
{
	int width = info->d_width;
	int height = info->d_height;
	Uint8 *src = info->s_pixels;
...
static void BlitNtoNPixelAlpha(MIL_BlitInfo *info)
{
	int width = info->d_width;
	int height = info->d_height;
	Uint8 *src = info->s_pixels;
...
MIL_loblit MIL_CalculateAlphaBlit(Surface *surface, int blit_index)
{
    MIL_PixelFormat *sf = surface->format;
    MIL_PixelFormat *df = surface->map->dst->format;
	...
	if (sf->BytesPerPixel == 4 && df->BytesPerPixel == 4 &&
	    !(surface->map->dst->flags & MIL_HWSURFACE) && MIL_HasAltiVec())
	...
	case 2:
#if MIL_ALTIVEC_BLITTERS
	if(sf->BytesPerPixel == 4 && !(surface->map->dst->flags & MIL_HWSURFACE) &&
           df->Gmask == 0x7e0 &&
	   df->Bmask == 0x1f && MIL_HasAltiVec())
