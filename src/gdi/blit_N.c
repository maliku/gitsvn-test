...
static void Blit_RGB888_RGB565(MIL_BlitInfo *info);
static void Blit_RGB888_RGB565Altivec(MIL_BlitInfo *info) {
...
static void Blit_RGB565_32Altivec(MIL_BlitInfo *info) {
    int height = info->d_height;
    Uint8 *src = (Uint8 *) info->s_pixels;
    int srcskip = info->s_skip;
    Uint8 *dst = (Uint8 *) info->d_pixels;
...
static void Blit_RGB555_32Altivec(MIL_BlitInfo *info) {
    int height = info->d_height;
    Uint8 *src = (Uint8 *) info->s_pixels;
    int srcskip = info->s_skip;
    Uint8 *dst = (Uint8 *) info->d_pixels;
...
static void BlitNtoNKey(MIL_BlitInfo *info);
static void BlitNtoNKeyCopyAlpha(MIL_BlitInfo *info);
static void Blit32to32KeyAltivec(MIL_BlitInfo *info)
{
    int height = info->d_height;
...
/* Use this on a G5 */
static void ConvertAltivec32to32_noprefetch(MIL_BlitInfo *info)
{
    int height = info->d_height;
...
/* Altivec code to swizzle one 32-bit surface to a different 32-bit format. */
/* Use this on a G4 */
static void ConvertAltivec32to32_prefetch(MIL_BlitInfo *info)
{
...
static Uint32 GetBlitFeatures( void )
{
    ...
    if (features == 0xffffffff) {
        ...
        char *override = MIL_getenv("MIL_ALTIVEC_BLIT_FEATURES");
        if (override) {
            features = 0;
            MIL_sscanf(override, "%u", &features);
        } else {
            features = ( 0
...
#else
/* Feature 1 is has-MMX */
#define GetBlitFeatures() ((Uint32)(MIL_HasMMX() ? 1 : 0))
#endif
...
#define HI	1
#define LO	0
#else /* MIL_BYTEORDER == MIL_BIG_ENDIAN */
#define HI	0
#define LO	1
#endif

#if MIL_HERMES_BLITTERS

/* Heheheh, we coerce Hermes into using MIL blit information */
#define X86_ASSEMBLER
#define HermesConverterInterface	MIL_BlitInfo
...
static void Blit_RGB888_index8(MIL_BlitInfo *info)
{
...
static void Blit_RGB888_RGB555(MIL_BlitInfo *info)
{
#ifndef USE_DUFFS_LOOP
	int c;
#endif
...
#endif /* MIL_HERMES_BLITTERS */


/* Special optimized blit for RGB 5-6-5 --> 32-bit RGB surfaces */
#define RGB565_32(dst, src, map) (map[src[LO]*2] + map[src[HI]*2+1])
static void Blit_RGB565_32(MIL_BlitInfo *info, const Uint32 *map)
{
#ifndef USE_DUFFS_LOOP
	int c;
#endif
...
static void Blit_RGB565_ARGB8888(MIL_BlitInfo *info)
{
...
static void Blit_RGB565_ABGR8888(MIL_BlitInfo *info)
{
...
static void Blit_RGB565_RGBA8888(MIL_BlitInfo *info)
{
...
static void Blit_RGB565_BGRA8888(MIL_BlitInfo *info)
{
...
#endif
static void Blit_RGB888_index8_map(MIL_BlitInfo *info)
{
#ifndef USE_DUFFS_LOOP
	int c;
#endif
...
static void BlitNto1(MIL_BlitInfo *info)
{
#ifndef USE_DUFFS_LOOP
	int c;
#endif
...
static void Blit4to4MaskAlpha(MIL_BlitInfo *info)
{
	int width = info->d_width;
	int height = info->d_height;
	Uint32 *src = (Uint32 *)info->s_pixels;
...
static void BlitNtoN(MIL_BlitInfo *info)
{
	int width = info->d_width;
	int height = info->d_height;
...
static void BlitNtoNCopyAlpha(MIL_BlitInfo *info)
{
	int width = info->d_width;
	int height = info->d_height;
	Uint8 *src = info->s_pixels;
...
static void BlitNto1Key(MIL_BlitInfo *info)
{
	int width = info->d_width;
	int height = info->d_height;
	Uint8 *src = info->s_pixels;
...
static void Blit2to2Key(MIL_BlitInfo *info)
{
	int width = info->d_width;
	int height = info->d_height;
...
MIL_loblit MIL_CalculateBlitN(Surface *surface, int blit_index)
{
	...
	if ( blit_index & 2 ) {
	        ...
	        return MIL_CalculateAlphaBlit(surface, blit_index);
	}
...
#if MIL_ALTIVEC_BLITTERS
        if((srcfmt->BytesPerPixel == 4) && (dstfmt->BytesPerPixel == 4) && MIL_HasAltiVec()) {
